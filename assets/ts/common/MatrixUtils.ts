import type {Vector} from 'assets/ts/common/gl/Vector';
 ...
 * @returns {Float32Array}
 */
export const initialize = (_mat: Float32Array): Float32Array => {
 ...
 */
export const multiply = (_mat1: Float32Array, _mat2: Float32Array, _dist: Float32Array = initialize(new Float32Array(16))): Float32Array => {
 ...
 * @returns {Float32Array}
 */
export const lookAt = (_targetPos: Vector, _cameraPos: Vector, _cameraUp: Vector, _dist: Float32Array): Float32Array => {
    // カメラの位置と見る地点が同じ場合は正方行列を返す
    if (_targetPos.x === _cameraPos.x && _targetPos.y === _cameraPos.y && _targetPos.z === _cameraPos.z) {
        return initialize(_dist);
    }
 ...
 * @returns {Float32Array}
 */
export const perspective = (_fov: number, _aspect: number, _near: number, _far: number, _dist: Float32Array): Float32Array => {
    // 近いクリップ面のy座標
 ...
 * @return Float32Array
 */
export const inverse = (_mat: Float32Array): Float32Array => {
    const inversed: Float32Array = create();
