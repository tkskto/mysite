...
export class Mesh {
    ...
    public ready = (_values: any[] = []) => {
        ...
        if (this._prg.uniType.length > 0) {
            GLUtils.setUniform(this._gl, this._prg.uniType, this._prg.unil, _values);
        }
    };
    ...
    public inverse = (): void => {
        const mat: Float32Array = this._mMatrix.copyWithin(0, 0);
        const a: number = mat[0],  b: number = mat[1],  c: number = mat[2],  d: number = mat[3],
        ...
        this._mMatrix[0]  = (f * B - g * A + h * z) * ivd;
        this._mMatrix[1]  = (-b * B + c * A - d * z) * ivd;
        this._mMatrix[2]  = (n * v - o * u + p * t) * ivd;
        this._mMatrix[3]  = (-j * v + k * u - l * t) * ivd;
        this._mMatrix[4]  = (-e * B + g * y - h * x) * ivd;
        this._mMatrix[5]  = (a * B - c * y + d * x) * ivd;
        this._mMatrix[6]  = (-m * v + o * s - p * r) * ivd;
        this._mMatrix[7]  = (i * v - k * s + l * r) * ivd;
        this._mMatrix[8]  = (e * A - f * y + h * w) * ivd;
        this._mMatrix[9]  = (-a * A + b * y - d * w) * ivd;
        this._mMatrix[10] = (m * u - n * s + p * q) * ivd;
        this._mMatrix[11] = (-i * u + j * s - l * q) * ivd;
        this._mMatrix[12] = (-e * z + f * x - g * w) * ivd;
        this._mMatrix[13] = (a * z - b * x + c * w) * ivd;
        this._mMatrix[14] = (-m * t + n * r - o * q) * ivd;
        this._mMatrix[15] = (i * t - j * r + k * q) * ivd;
    };
