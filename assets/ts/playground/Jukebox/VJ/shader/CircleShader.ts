...
float getfrequency(float x) {
    return texture2D(audio, vec2(floor(x * FREQ_RANGE + 1.0) / FREQ_RANGE, 0.0)).x + 0.06;
}
...
void main() {
    ...
    // dot
    // vec3 color = vec3(0.0134, 0.052, 0.1);
    for (float i = .0; i< 0.99; i+=.05) {
        colorize += texture2D(audio, vec2(i,0.0)).y * pow(i+.5,.9);
    }

    colorize = normalize(colorize);
    float boost = texture2D(audio,vec2(.0)).x;
    ...
    float blockint = texture2D(audio, blocksid,-48.0).y;
    float oint = blockint = - texture2D(audio,vec2(blockint-.02,.0)).x+2.0*texture2D(audio,vec2(blockint,.0)).x - texture2D(audio,vec2(blockint+.02,.0)).x;
    blockint = 1.0;
