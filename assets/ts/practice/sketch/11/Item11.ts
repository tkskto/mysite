...
export default class Item11 extends Sketch {
    ...
    public setup = async (): Promise<any> => {
        ...
        GLUtils.createTexture('../../../../img/practice/11_1.jpg', this._gl, this._gl.UNSIGNED_BYTE).then(async tex => {
            this._mesh.addTexture(tex);
            return GLUtils.createTexture('../../../../img/practice/11_2.jpg', this._gl, this._gl.UNSIGNED_BYTE);
        }).then(tex => {
            this._mesh.addTexture(tex);
            this.play();
        }).catch(err => {
