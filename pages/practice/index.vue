...
import Item20 from '~/assets/ts/practice/sketch/20/Item20';
...
useHead({
    ...
    meta: [
        {
            name: 'description',
            content: 'This is practice of GLSL and part of portfolio.',
        },
    ],
});

const {screenSize, canvasSize} = useScreenSize();

const canvasDisplayWidth = computed(() => {
    if (canvasSize) {
        return canvasSize.value.width;
    }
    return 0;
});
...
const canvasDisplayHeight = computed(() => {
    if (canvasSize) {
        return canvasSize.value.height;
    }
    return 0;
});
...
const canvasPracticalWidth = computed(() => {
    if (canvasSize) {
        return `${canvasSize.value.width / window.devicePixelRatio}px`;
    }
    return 0;
});
...
const canvasPracticalHeight = computed(() => {
    if (canvasSize) {
        return `${canvasSize.value.height / window.devicePixelRatio}px`;
    }
    return 0;
});
